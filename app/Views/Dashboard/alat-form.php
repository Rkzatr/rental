...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <a href="<?= base_url('alat') ?>" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
        <i class="fas fa-hand-point-left fa-sm text-white-50"></i>
        Kembali</a>
        ...
        <div class="card border-left-primary shadow h-100 py-2">
            <div class="card-body">
                <form class="my-form" method="POST"
                    action="<?= $item ? base_url("api/alat/{$item->id}") : base_url('api/alat') ?>"
                    enctype="multipart/form-data">
                    <fieldset>
                        <div class="form-group">
                            <label for="nama">Nama Alat</label>
                            <input type="text" id="nama" name="nama" class="form-control"
                                value="<?= $item ? $item->nama : '' ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="deskripsi">Deskripsi</label>
                            <textarea name="deskripsi" id="deskripsi" class="form-control"
                                rows="5"><?= $item ? $item->deskripsi : '' ?></textarea>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="harga" name="harga" class="form-control"
                                value="<?= $item ? $item->harga : '' ?>" required>
                        </div>
                        <div class="form-group">
                            <label for="stok">Stok</label>
                            <input type="number" id="stok" name="stok" class="form-control"
                                value="<?= $item ? $item->stok : '' ?>" required>
                        </div>
                        <?php if ($item): ?>
                        <div class="row justify-content-center">
